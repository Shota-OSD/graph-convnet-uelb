...
def get_default_config():
    ...
    """
    # Try to find default.json in gcn configs first, then fall back to empty config
    import os
    default_paths = [
        "./configs/gcn/default.json",
        "./configs/default.json"
    ]
    for path in default_paths:
        if os.path.exists(path):
            return Settings(json.load(open(path)))
    # If no default found, return empty settings
    return Settings({})
