...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/default.json',
                       help='ベース設定ファイルのパス (default: configs/default.json)')
    parser.add_argument('--tuning-config', type=str, default='configs/tuning_config.json',
                       help='チューニング設定ファイルのパス (default: configs/tuning_config.json)')
    ...
    if not _check_data_exists():
        ...
        print("Please run 'python generate_data.py' first to create training data.")
        sys.exit(1)
...
def _create_best_config(best_result, base_config_path, results_dir):
    ...
    print(f"Use this config with: python main.py --config {best_config_path}")
